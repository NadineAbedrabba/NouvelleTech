...
export class ReviewsComponent implements OnInit {
  ...
  loadReviews() {
    console.log('Loading reviews for ID:', this.entrepriseId);
    ...
    this.reviewService.getReviewsByEntreprise(this.entrepriseId).subscribe({
      next: (data) => {
        ...
        this.reviews.forEach(review => {
          console.log(`Review ID ${review.id}: clientPhoto = ${review.clientPhoto}`);
        });
        this.cdRef.detectChanges();
      },
      error: (err) => {
  ...
  getClientPhoto(photoUrl: string | null | undefined): string {
    ...
    if (photoUrl.startsWith('http://') || photoUrl.startsWith('https://')) {
      ...
      return photoUrl; // No timestamp for external URLs to avoid change detection issues
    }
